...
; RUN: %if webassembly-registered-target %{ opt -mtriple=wasm32-unknown-unknown                         -passes=pre-isel-intrinsic-lowering -S %s | FileCheck %s --check-prefixes=COMMON,INTACT %}
; RUN: %if webassembly-registered-target %{ opt -mtriple=wasm32-unknown-unknown -mcpu=mvp               -passes=pre-isel-intrinsic-lowering -S %s | FileCheck %s --check-prefixes=COMMON,EXPAND %}
...
define void @memset_n(ptr %dest, i8 %v, i64 %n) {
; INTACT-LABEL: @memset_n(
; INTACT-NEXT:    call void @llvm.memset.inline.p0.i64(ptr {{.*}}, i8 {{.*}}, i64 {{.*}}, i1 false)
; INTACT-NEXT:    ret void
;
; EXPAND-LABEL: @memset_n(
; EXPAND-NOT:   call void @llvm.memset.inline
; EXPAND:       store i8
  call void @llvm.memset.inline.p0.i64(ptr %dest, i8 %v, i64 %n, i1 false)
  ret void
}
