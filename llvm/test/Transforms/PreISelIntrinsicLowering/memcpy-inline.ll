...
; RUN: %if webassembly-registered-target %{ opt -mtriple=wasm32-unknown-unknown -mattr=-bulk-memory,-bulk-memory-opt -passes=pre-isel-intrinsic-lowering -S %s | FileCheck %s --check-prefixes=COMMON,EXPAND %}
...
define void @memcpy_n(ptr %dst, ptr %src, i64 %n) {
; INTACT-LABEL: @memcpy_n(
; INTACT-NEXT:    call void @llvm.memcpy.inline.p0.p0.i64(ptr {{.*}}, ptr {{.*}}, i64 {{.*}}, i1 false)
; INTACT-NEXT:    ret void
;
; EXPAND-LABEL: @memcpy_n(
; EXPAND-NOT:   call void @llvm.memcpy.inline
; EXPAND:       store i8
