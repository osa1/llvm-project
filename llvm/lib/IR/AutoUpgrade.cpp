...
static bool upgradeArmOrAarch64IntrinsicFunction(bool IsArm, Function *F,
  ...
  if (IsArm) {
    ...
    if (Neon) {
      ...
      // Changed in 24.0: the integer min/max intrinsics are exactly equivalent
      // to the target-independent ones, so use those instead. Note that the
      // trailing '.' keeps this from matching the across-lane ('sminv') and
      // pairwise ('sminp') intrinsics, which have no generic equivalent.
